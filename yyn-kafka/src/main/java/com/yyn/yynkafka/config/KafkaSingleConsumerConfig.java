...
public class KafkaSingleConsumerConfig {
    ...
    @Value("${test_single_topic.group}")
    private String groupId;
    @Bean(name = "singleConsumerConfigs")
...
//
    ...
    public ConsumerFactory<Object, Object> singleConsumerFactory() {
        //配置消费者的 Json 反序列化的可信赖包，反序列化实体类需要
        try(JsonDeserializer<Object> deserializer = new JsonDeserializer<>()) {
            deserializer.trustedPackages("*");
            return new DefaultKafkaConsumerFactory<>(singleConsumerConfigs(), new JsonDeserializer<>(), deserializer);
        }
    }

    @Bean(name = "singleKafkaListenerContainerFactory")
    public KafkaListenerContainerFactory<ConcurrentMessageListenerContainer<Object, Object>> singleKafkaListenerContainerFactory() {
